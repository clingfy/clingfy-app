...
class WorkflowEventPublisher {
 public:
  ...
  // `code` (Phase 10.2, optional) is a stable machine-readable id letting
  // Dart localize the toast and attach the right settings deep link. Since
  // Phase 10.4 the C++ constants live in `Bridge/recording_warning_codes.h`
  // (clingfy::bridge::warning) — emit sites use those instead of repeated
  // string literals. Empty = legacy message-only payload (macOS localizes
  // natively and never sends a code). Keep that header in sync with the
  // constants in lib/core/bridges/recording_warning_codes.dart and the
  // code→toast mapping in lib/app/home/home_error_mapper.dart.
