...
| 18 | Voice cleanup — gating | Open a system-audio-only recording (no mic) in post → Audio | Voice Cleanup control hidden and the "no mic audio" notice shown; a separated mic recording shows the control (depends on §2 mic+system separation working) | — (on-device pending) |
| 19 | Editing — clips / reorder / mix | Cut + delete a middle segment then replay across the cut; drag-reorder clips then play and export; raise mic-only gain on a separated recording | Video stays locked to audio across the cut (no lag); reorder plays/export in timeline order; gain raises the mic only, system steady | — (on-device pending) |
