...
- **The caption track gets undo and redo.** Correcting a cue could not be undone at all, while the bar directly above the panel offers Undo/Redo for Zoom, Clips and Color — so the absence read as a broken feature rather than an unbuilt one. The machine's original wording is gone the moment you type over it, and until now the only recovery was retyping from memory, or "Generate again", which destroys every other correction too.
- **Subtitles tell you when a cue was too long to fit the frame.** Burned-in cue text is capped at three lines with an ellipsis, and the speech model routinely emits a whole sentence per segment — so a long cue was drawn cut off in the video while the `.srt`/`.vtt` beside it and the caption editor both kept the full sentence, with nothing said about the difference. Portrait is the worst case, because the font scales with the canvas height while the wrap box is a fraction of its width.
...
- **A GIF export no longer writes `.srt` and `.vtt` beside it.** Choosing Subtitle file (or Both) and then picking GIF produced sidecars that no GIF viewer, browser or platform can load — inert files whose presence suggested the GIF was captioned when the sidecar half of that request cannot be honoured at all. Burn in is the destination that works for a GIF, and it does.
- **A `<` in cue text no longer swallows the rest of the cue.** WebVTT reads `<` as the start of a tag, so a cue corrected to `if x < y then` rendered as `if x ` in a browser, on YouTube and in VLC — while the `.vtt` still looked perfect in a text editor, which is what kept it invisible until somebody watched the result. Recordings of code are a core use case here, so a `<` in a cue is ordinary rather than exotic.
- **An empty transcript no longer writes empty subtitle files while reporting success.** The captions panel has no delete affordance, so clearing the text of every cue is how a transcript gets removed — and that produced a 0-byte `.srt` beside the video with the export still saying it had succeeded.
- **Windows: subtitle sidecars no longer land one folder above the video.** The path-splitting looked for the platform separator only, so on Windows every forward slash was invisible to it — and real paths here mix both, because native returns backslashes while paths joined in Dart carry forward slashes. A video in `My.Videos` had its sidecars written a directory up, under the folder's name.
- **Captions no longer refuse the next run immediately after finishing one.** The service went on reporting itself busy for a moment after telling the caller the job had finished, so asking to generate again from that point was refused with "A transcription is already running".
- **A caption correction is no longer lost when the panel goes away**, and a correction no longer resets the track it belongs to.
- **A failed captions capability probe now says so** instead of the feature quietly vanishing from the panel with no reason given.
- **Windows: starting a recording right after stopping one no longer hangs.** A lock cycle between the recording indicator and the engine, deterministic rather than a race.
- **Reopening a project that has a transcript no longer restores its canvas on defaults**, and a queued canvas write is now saved to the project it was queued for rather than whichever project happens to be open when it lands.
...
- Captions: a long transcript no longer inflates every cue field, and CI now fails when a `RunnerTests` class is silently not run.
